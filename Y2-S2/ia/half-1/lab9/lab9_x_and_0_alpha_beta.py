...
def alpha_beta(alpha, beta, stare):
    ...
    if stare.j_curent == Joc.JMAX:
        ...
        scor_curent = -0x3f3f3f3f
        for mutare in stare.mutari_posibile:
            stare_fiu = alpha_beta(alpha, beta, mutare)
            if stare_fiu.scor > scor_curent:
                scor_curent = stare_fiu.scor
                if stare_fiu.scor > alpha:
                    alpha = stare_fiu.scor
        ...
        scor_curent = 0x3f3f3f3f
        for mutare in stare.mutari_posibile:
            stare_fiu = alpha_beta(alpha, beta)
            if stare_fiu.scor < scor_curent:
                scor_curent = stare_fiu.scor
                if stare_fiu.scor < beta:
                    beta = stare_fiu.scor
                    stare.stare_aleasa = mutare
                    if alpha >= beta:
                        break
