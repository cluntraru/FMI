...
fibonacciLiniar 0 = 0
fibonacciLiniar n =
    snd (fibonacciPereche n)
    where
        fibonacciPereche :: Integer -> (Integer, Integer)
        fibonacciPereche 1 = (0, 1)
        fibonacciPereche n = 
            let fiboPrevPair = fibonacciPereche (n - 1)
                fiboPrevFst = fst(fiboPrevPair)
                fiboPrevSnd = snd(fiboPrevPair)
            in (fiboPrevSnd, fiboPrevFst + fiboPrevSnd)
